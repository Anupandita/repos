...
namespace Sfc.Wms.Api.Asrs.Test.Integrated.Fixtures
{
    public class DataBaseFixtureForOrmt : CommonFunction
    {
        ...
        protected string CommonSelect = "select distinct eg.CARTON_NBR,eg.CREATED_DATE_TIME,ch.wave_nbr,ch.sku_id,pl.locn_id,ch.MISC_NUM_1," +
        ...
        protected string CommonJoin  = "inner join carton_hdr ch on ch.Carton_Nbr = eg.Carton_Nbr inner join PKT_HDR ph ON ph.pkt_ctrl_nbr = ch.pkt_ctrl_nbr " +
        ...
        protected string CommonWhereCondition = "where  ch.misc_instr_code_5 is null and misc_num_1 = :miscNum1 and sc.code_type = :sysType " +
       ...
       public void GetValidDataBeforeTriggerOrmtForPrintingOfCartonsThroughWaveNumber()
       {
        ...
        public CartonView GetValidOrderDetails(OracleConnection db, int statCode, int miscNum1)
        {
            ...
            var query = $"{CommonSelect} from SWM_ELGBL_ORMT_CARTONS eg " +
                        $"{CommonJoin}"+                   
            ...
            //command.Parameters.Add(new OracleParameter("statCode", Constants.PrintCartonStatus));
        ...
        public CartonView GetValidOnProcessCostMessage(OracleConnection db)
        {
            ...
            var onQuery = $"{CommonSelect} from swm_from_mhe sfm " +    
                $"inner join SWM_ELGBL_ORMT_CARTONS eg on eg.carton_nbr = ch.carton_nbr " +
                $"{CommonJoin}"+
                $"{CommonWhereCondition} and sfm.source_msg_trans_code = '{TransactionCode.Cost}' ORDER BY eg.CARTON_NBR,eg.CREATED_DATE_TIME";
        ...
        public CartonView GetCartonNbrWhereActiveOrmtNotFound(OracleConnection db)
        {
            ...
            var query = $"select ch.carton_nbr,ch.sku_id,ch.wave_nbr,ch.MISC_NUM_1, ch.total_qty, ch.stat_code,pl.actl_invn_qty,ch.DEST_LOCN_ID,ph.PKT_CTRL_NBR," +
                ...
                $"where sc.code_type = '{Constants.SysCodeType}' and code_id = '{Constants.SysCodeIdForActiveLocation}' and " +
