namespace Sfc.Wms.Api.Asrs.Test.Integrated.TestData
{
    public class OrmtQueries
    {
        ...
        public static string WaveRelease = $"select distinct ch.carton_nbr,ch.create_date_time,ch.wave_nbr,ch.temp_zone,ch.sku_id,pl.locn_id,ch.MISC_NUM_1, ch.total_qty,ch.stat_code,pl.actl_invn_qty,ch.DEST_LOCN_ID,ph.PKT_CTRL_NBR,ph.SHIP_W_CTRL_NBR,ph.Whse, ph.CO,ph.DIV, " +
                                           ...
                                           $" inner join pkt_dtl pd ON pd.pkt_ctrl_nbr = ch.pkt_ctrl_nbr inner join alloc_invn_dtl ai ON ai.sku_id = ch.sku_id left join pick_locn_dtl_ext ple ON pl.sku_id = ple.sku_id where sc.code_type = '740' and code_id = '18' and misc_num_1 = '0'  and eg.status = '0' and ch.stat_code = '5' and eg.wave_nbr = '20200115001'";

        public static string ValidCartonsForAddRelease = $"select distinct ch.carton_nbr,ch.create_date_time,ch.temp_zone,ch.wave_nbr,ch.sku_id,pl.locn_id,ch.MISC_NUM_1, ch.total_qty," +
