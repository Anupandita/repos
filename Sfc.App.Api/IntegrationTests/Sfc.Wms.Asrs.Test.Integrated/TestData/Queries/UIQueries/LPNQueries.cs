...
namespace FunctionalTestProject.SQLQueries
{
    public static class LpnQueries
    {
        ...
        public static string FetchLpnPageGridDtSql() => $@"SELECT ch.CASE_NBR caseNumber,ch.RCVD_SHPMT_NBR receivedShipmentNumber ,DSP_LOCN displayLocation,cd.SKU_ID skuId,SKU_DESC skuDescription,DC_ORD_NBR distributionCenterOrderNumber,LOCK_CNT lockCount,
                   CASE WHEN lock_cnt > 0 THEN 'Y' ELSE '' END AS lockCount_1,CONS_CASE_PRTY consumeCasePriority, to_char(CONS_PRTY_DATE,'{UIConstants.FormatDateTime}') consumePriorityDate,CONS_SEQ consumeSequence,
                   to_char(ch.MFG_DATE,'{UIConstants.FormatDateTime}') manufacturingOn,to_char(RCVD_DATE,'{UIConstants.FormatDateTime}') receivedOn,to_char(ch.XPIRE_DATE,'{UIConstants.FormatDateTime}') expiryDate,
                   ch.PROC_IMMD_NEEDS processImmediateNeeds,ltrim(to_char(VOL,'{UIConstants.VolumeDecimalFormat}')) volume,ltrim(to_char(EST_WT,'{UIConstants.DecimalFormat}')) estimateWeight,ltrim(to_char(ACTL_WT,'{UIConstants.DecimalFormat}')) actualWeight,ch.PO_NBR poNumber,get_sc_desc ('B','509',ch.stat_code,NULL) codeDescription ,
                   ...
                   ch.SPL_INSTR_CODE_3 specialInstructionCode3,ch.SPL_INSTR_CODE_4 specialInstructionCode4,ch.SPL_INSTR_CODE_5 specialInstructionCode5,TOTAL_ALLOC_QTY totalAllocatedQuantity, ORIG_QTY originalQuantity,
                   ACTL_QTY actualQuantity,SHPD_ASN_QTY shippedAsnQuantity,to_char(ch.CREATE_DATE_TIME,'{UIConstants.FormatDateTime}') createdOn,to_char(ch.MOD_DATE_TIME,'{UIConstants.FormatDateTime}') updatedOn,CASE WHEN cm.cmnt_seq_nbr is null THEN 'N' ELSE 'Y' END AS ""comment"",
                   sw.First_name || ' ' || sw.last_name userName,ad.vendor_item_nbr vendorItemNumber, cd.cut_nbr cutNumber, ad.units_rcvd-ad.units_shpd varianceQuantity FROM CASE_HDR ch INNER JOIN CASE_DTL cd ON cd.case_nbr = ch.case_nbr
        ...
        {
            return $@"SELECT event,to_char(audit_datetime,'{UIConstants.FormatDateTime}') auditDateTime,audit_seq_nbr,case_nbr lpn,getlocation (whse, bf_locn_id) beforeLocationId,
                       ...
                       s2.first_name ||' '||s2.last_name afterUser,af_plt_id afterPalletId,audit_seq_nbr auditSequenceNumber,bf_rsn_code beforeReasonCode, af_rsn_code afterReasonCode,case_nbr,af_cons_case_prty afterConsCasePriority,af_db_user afterDbUser,to_char(af_cons_prty_date,'{UIConstants.FormatDateTime}') afterConsPriorityDate FROM sfc_audit_case_hdr sc left outer join swm_user_master s1 on s1.user_name=sc.bf_user_id left outer join swm_user_master s2 on s2.user_name=sc.af_user_id WHERE whse = '{UIConstants.Whse}'
                       AND case_nbr = '{UIConstants.LpnNumberForHistory}' order by event";
        }
        ...
        {
            return $@"Select cl.invn_lock_code || ':'|| get_sc_desc('B','527',cl.invn_lock_code,NULL) inventoryLockCode, cm.cmnt lockComment, to_char(cl.create_date_time,'{UIConstants.FormatDateTime}') lockDateTime from case_lock cl 
        ...
        public static string FetchCaseCommentsDtSql() => $@"select cm.cmnt_type || ' - ' ||get_sc_desc ('B','346',cm.cmnt_type,NULL) commentType ,cm.cmnt_code || ' - '||get_sc_desc ('B','347',cm.cmnt_code,NULL) commentCode , cm.cmnt ""commentText"" from case_cmnt cm 
