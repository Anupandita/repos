...
namespace Sfc.Wms.Api.Asrs.Test.Integrated.TestData
{
    ...
    {
        public const string CommonJoin = "inner join CASE_DTL on CASE_HDR.CASE_NBR = CASE_DTL.CASE_NBR inner join PICK_LOCN_DTL pl on CASE_DTL.SKU_ID = pl.SKU_ID inner join ITEM_MASTER im ON CASE_DTL.sku_id = im.sku_id inner join locn_hdr lh ON CASE_HDR.locn_id = lh.locn_id  inner join locn_grp lg ON lg.locn_id = lh.locn_id inner join sys_code sc ON sc.code_id = lg.grp_type ";
        public const string ReceivedCaseFromReturns = "select CASE_HDR.CASE_NBR,CASE_HDR.LOCN_ID,CASE_HDR.STAT_CODE from CASE_HDR "+ CommonJoin +"  " +
        ...
        public const string ReceivedCaseFromVendors = "select CASE_HDR.CASE_NBR,CASE_HDR.LOCN_ID,CASE_HDR.STAT_CODE from  CASE_HDR " +
        ...
        public const string CaseHdrDtlJoin = "select case_hdr.stat_code,case_dtl.total_alloc_qty,case_dtl.actl_qty from case_hdr " +
                "inner join case_dtl on case_hdr.case_nbr = case_dtl.case_nbr and case_hdr.case_nbr = :caseNumber";
        public const string CaseHdrDtlTransInvnJoin = "select TRANS_INVN.MOD_DATE_TIME, CASE_HDR.STAT_CODE,CASE_DTL.ACTL_QTY,CASE_DTL.TOTAL_ALLOC_QTY,TRANS_INVN.ACTL_INVN_UNITS,TRANS_INVN.ACTL_WT from CASE_HDR inner join CASE_DTL on CASE_HDR.CASE_NBR = CASE_DTL.CASE_NBR  " +
        ...
        public const string TransInvnForMultiSku = "select sku_id,locn_id,trans_invn_type,actl_invn_units,actl_wt,user_id,mod_date_time from " +
