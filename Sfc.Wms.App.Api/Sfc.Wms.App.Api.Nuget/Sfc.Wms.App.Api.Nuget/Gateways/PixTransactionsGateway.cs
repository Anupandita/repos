...
namespace Sfc.Wms.App.Api.Nuget.Gateways
{
    public class PixTransactionsGateway : SfcBaseGateway,IPixTransactionsGateway
    {
        ...
        public async Task<BaseResult<string>> GetPixTransactionsDetailsAsync(PixTranactionsModel pixTranactionsModel, string token)
        {
        ...
        {
            var resource = $"{_endPoint}/{Routes.Paths.SearchPixTransactions}{Routes.Paths.QueryParamSymbol}pageNo={pixTranactionsModel.pageNo}{Routes.Paths.QueryParamAnd}rowsPerPage={pixTranactionsModel.rowsPerPage}{Routes.Paths.QueryParamAnd}totalRows={pixTranactionsModel.totalRows}";
            resource = QueryStringBuilder.BuildQuery("inpt_itemid=", pixTranactionsModel.inpt_itemid, resource, false);
            resource = QueryStringBuilder.BuildQuery("inpt_start_date=", pixTranactionsModel.inpt_start_date, resource, false);
            resource = QueryStringBuilder.BuildQuery("inpt_end_date=", pixTranactionsModel.inpt_end_date, resource, false);
            resource = QueryStringBuilder.BuildQuery("inpt_nbr_days=", pixTranactionsModel.inpt_nbr_days, resource, false);
